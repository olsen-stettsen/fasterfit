...
function posttodb(username, email, password){
  ...
  client.connect();
  client.query('INSERT INTO account (user_name, user_email, user_password) VALUES (\'' + username + '\',\'' + email + '\',\'' + password+ '\') WHERE NOT EXISTS (SELECT user_name FROM account WHERE user_name = \'' + username + '\');', (err, res) => {
    var results = [];
    if (err) throw err;
    for (let row of res.rows) {
      results.push(row);
